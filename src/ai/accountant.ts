...
profile (обновление данных профиля питомца: имя, порода, вес, дата рождения, окрас, пол):
{ "module": "profile", "confidence": 0.9, "data": { "name": "имя питомца", "breed": "порода", "weight": 4.5, "birthDate": "YYYY-MM-DD", "color": "окрас", "gender": "male"|"female" } }
...
profile (update pet profile: name, breed, weight, date of birth, colour, gender):
{ "module": "profile", "confidence": 0.9, "data": { "name": "pet name", "breed": "breed", "weight": 4.5, "birthDate": "YYYY-MM-DD", "color": "colour", "gender": "male"|"female" } }
...
export async function parseUserText(text: string, pet: Pet): Promise<ParsedAtom[]> {
  const setupContext = !pet.name
    ? appLang === 'ru'
      ? `Контекст: владелец только что завёл питомца и отвечает на вопрос "Как зовут питомца?". Следующее сообщение — это имя питомца.\n`
      : `Context: the owner just added a pet and is answering the question "What is the pet's name?". The following message is the pet's name.\n`
    : '';
  const prompt = appLang === 'ru'
    ? `${setupContext}Питомец: ${pet.name || '(имя не задано)'} (${pet.species}${pet.breed ? ', ' + pet.breed : ''})\nСообщение: "${text}"\n\nВерни JSON массив атомов.`
    : `${setupContext}Pet: ${pet.name || '(no name yet)'} (${pet.species}${pet.breed ? ', ' + pet.breed : ''})\nMessage: "${text}"\n\nReturn a JSON array of atoms.`;
